...
class _LiveMatchesScreenState extends State<LiveMatchesScreen> {
  late final Future<List<dynamic>> _matches = ApiClient().getList('/admin/live-matches/');
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<dynamic>>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) return Center(child: Text('Matchs indisponibles: ${snapshot.error}'));
          final matches = snapshot.data ?? [];
          if (matches.isEmpty) return const Center(child: Text('Aucun match en direct dans Supabase.'));
          return ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                ...
                title: Text(match['tournament_name']?.toString() ?? 'Table ${index + 1}'),
                subtitle: Text('${match['white_username'] ?? 'Blancs'} vs ${match['black_username'] ?? 'Noirs'}'),
