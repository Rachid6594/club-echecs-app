...
import { ApiState, DashboardPayload } from '../lib/admin-api';
import { adminClientFetch } from '../lib/admin-client';

export default function AdminDashboard() {
  const [state, setState] = useState<ApiState<DashboardPayload> | null>(null);

  useEffect(() => {
    adminClientFetch<DashboardPayload>('/admin/dashboard/').then(setState);
  }, []);
  ...
  return (
    <AdminShell>
      ...
      {!state ? <StatusMessage title="Chargement" detail="Lecture securisee du dashboard..." /> : !state.ok ? (
        <StatusMessage title="Connexion admin requise" detail={state.error} />
      ) : (
        <>
